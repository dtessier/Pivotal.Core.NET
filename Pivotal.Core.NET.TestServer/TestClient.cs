...
namespace Pivotal.Core.NET.TestServer {
   public class TestClient {
        ...
        public void Go() {
			...
			client.BufferCodec.ScanForIdentity ("Pivotal.Core.NET", "^Pivotal.Core.NET.*Command$");
			client.BufferCodec.ScanForIdentity ("Pivotal.Core.NET.TestServer", "^Pivotal.Core.NET.*Command$");
		...
		protected void OnWhatTimeIsIt(IClientSocket client, ICommand command) {
			...
			Console.Out.WriteLine (String.Format ("< Server replied back with time is {0} for client {1}", wtit.TheTime, client));
		}
        ...
        protected void OnConnected(IClientSocket client) {
			...
			System.Diagnostics.Debug.WriteLine (String.Format ("Test Client Connected {0}", client));

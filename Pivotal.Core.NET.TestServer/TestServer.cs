...
namespace Pivotal.Core.NET.TestServer
{
	...
	public class TestServer : AbstractServer {
		...
		protected void OnWhatTimeIsIt(IClientSocket client, ICommand command) {
			...
			Console.Out.WriteLine (String.Format ("> Server received what time it is request from {0}", client));
		...
		protected override void OnStarted() {
			...
			DefaultBufferCodec.ScanForIdentity ("Pivotal.Core.NET", "^Pivotal.Core.NET.*Command$");
			DefaultBufferCodec.ScanForIdentity ("Pivotal.Core.NET.TestServer", "^Pivotal.Core.NET.*Command$");

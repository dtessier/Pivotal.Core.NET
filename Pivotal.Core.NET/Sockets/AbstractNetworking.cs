...
namespace Pivotal.Core.NET.Sockets {
	...
	public abstract class AbstractNetworking {
		...
		protected virtual bool AttemptReceiveEventHandler(IClientSocket client, ICommand command, bool async) {
			if (command == null) {
				Debug.WriteLine (String.Format ("Invalid command was received, null pointer for client {0}", client));
			...
			if (!HandlerEventCommands.ContainsKey (type)) {
				Debug.WriteLine (String.Format ("Command {0} with serial {1} received for Socket {2} on " +
					"Server type {3} but no OnReceiveDelegate event handler found for command type", command, type, client, this)
				);
			...
			if (async) {
				deleg.BeginInvoke (client, command, BeginInvokeReceiveEventHandlerCallback, deleg);
			} else {

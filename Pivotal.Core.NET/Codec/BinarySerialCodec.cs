		...
		public override void AddIdentity(CommandIdentifier identity) {
			...
			if (!(typeof(IBinarySerializable).IsAssignableFrom(identity.CommandType))) {
				throw new InvalidCastException(
					String.Format ("Cannot register command {0} to type {1}, make sure your command implements {2}", 
				    	identity.CommandType, typeof(IBinarySerializable), identity.CommandType));
